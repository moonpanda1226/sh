...
public class Logout extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * 세션 쿠기
		 * 세션 : 서버에 저징됩니다.					쿠키 : 클라이언트에 저장(브라우저)
		...
		if(session.getAttribute("mname") != null) {
			...
			//System.out.println("mname : " + session.getAttribute("maname"));
		...
		session.invalidate();
		//invalidate() 는 세션자체를 무효화하고 제거하고
		//removeAttribute()는 현재 세션에서 특정 key-value 만 제거를 한다.
		//removeAttribute()로 키만 제거를 하면 httpSession 인스턴스가 남아있다.
		//invalidate() 해주는것이 좋습ㄴ다.
